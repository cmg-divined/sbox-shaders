...
PS
{
    ...
    int bits = 8; // Change this to 16, 32, etc. for different bit depths
    float maxColor = pow(2, bits) - 1;
    color.rgb = floor(color.rgb * maxColor) / maxColor;

    // Add shadow highlight effect
    float shadowHighlight = 1.0; // Adjust this value from 0 to 1
    float3 reducedColor = floor(color.rgb * maxColor * shadowHighlight) / maxColor;
    color.rgb = lerp(reducedColor, color.rgb, shadowHighlight);
